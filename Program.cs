...
namespace SistemaExamenes
{
    ...
    public abstract class ExamenBase : IPrototipo
    {
        ...
        public virtual IPrototipo Clonar()
        {
            ...
            clon._idExamen = GenerarId();   // ID único por clon
            return clon;
        }
    ...
    public class RegistroPrototipos
    {
        ...
        public void Registrar(string clave, ExamenBase proto)
        {
            ...
            Console.WriteLine(
                $"  ✅  [{clave,-18}]  {proto.NombreAsignatura,-38}  Docente: {proto.Docente}");
        }
        ...
        public void Listar()
        {
            ...
            foreach (var p in _prototipos)
                Console.WriteLine($"       {p.Key,-20}  →  {p.Value}");
            Console.WriteLine();
        }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 1. Patrones de Diseño  —  Maribel Guerrero Luis
            var protoPD = new ExamenPatronesDiseno("Maribel Guerrero Luis", "C-201");
            ...
            // 2. Cálculo Diferencial  —  Zyanya Anaya
            var protoCalc = new ExamenCalculoDiferencial("Zyanya Anaya", "A-105");
            ...
            // 3. Bases de Datos  —  Gabriela Tapia
            var protoBD = new ExamenBasesDatos("Gabriela Tapia", "B-310");
            ...
            // 4. Física  —  Zyanya Anaya (mismo docente que Cálculo)
            var protoFis = new ExamenFisica("Zyanya Anaya", "A-208");
            protoFis.Unidad = 2;
            ...
            // 5. POO  —  Maribel Guerrero Luis  (mismo docente que Patrones)
            var protoPOO = new ExamenPOO("Maribel Guerrero Luis", "C-202");
            protoPOO.Lenguaje = "C#";
            ...
            // 6. Inglés  —  Lic. Williams Turner
            var protoIng = new ExamenIngles("Lic. Williams Turner", "D-102");
            ...
            // 7. Redes de Computadoras  —  Ricardo Palma
            var protoRed = new ExamenRedes("Ricardo Palma", "B-201");
            ...
            // 8. Inteligencia Artificial  —  Arnulfo Alanis 
            var protoIA = new ExamenIA("Arnulfo Alanis", "C-305");
            ...
            // ── Registrar los 8 ─────────────────────────────────────
            registro.Registrar("PD_Guerrero", protoPD);
            registro.Registrar("CALC_Anaya", protoCalc);
            registro.Registrar("BD_Tapia", protoBD);
            registro.Registrar("FISICA_Anaya", protoFis);
            registro.Registrar("POO_Guerrero", protoPOO);
            registro.Registrar("INGLES", protoIng);
            registro.Registrar("REDES_Palma", protoRed);
            registro.Registrar("IA_Alanis", protoIA);
            ...
            var pdA = registro.ObtenerClon("PD_Guerrero");
            pdA.Grupo = "A"; pdA.Salon = "C-201"; pdA.Fecha = "2024-11-05";
            examenes.Add(pdA);

            var pdB = registro.ObtenerClon("PD_Guerrero");
            pdB.Grupo = "B"; pdB.Salon = "C-202"; pdB.Fecha = "2024-11-06";
            examenes.Add(pdB);

            var pdC = registro.ObtenerClon("PD_Guerrero");
            pdC.Grupo = "C"; pdC.Salon = "C-203";
            pdC.Docente = "Perez Castro";
            examenes.Add(pdC);

            var calcA = registro.ObtenerClon("CALC_Anaya");
            calcA.Grupo = "A"; calcA.Salon = "A-105";
            examenes.Add(calcA);

            var calcB = registro.ObtenerClon("CALC_Anaya");
            calcB.Grupo = "B"; calcB.Salon = "A-107";
            examenes.Add(calcB);

            var calcC = registro.ObtenerClon("CALC_Anaya");
            calcC.Grupo = "C"; calcC.Salon = "A-109";
            calcC.Docente = "De La Rosa";
            examenes.Add(calcC);

            var bdA = registro.ObtenerClon("BD_Tapia");
            bdA.Grupo = "A"; bdA.Salon = "B-310";
            examenes.Add(bdA);

            var bdB = registro.ObtenerClon("BD_Tapia");
            bdB.Grupo = "B"; bdB.Salon = "B-312";
            ((ExamenBasesDatos)bdB).MotorBD = "MySQL";
            examenes.Add(bdB);

            var fisA = registro.ObtenerClon("FISICA_Anaya");
            fisA.Grupo = "A"; fisA.Salon = "A-208";
            examenes.Add(fisA);

            var fisB = registro.ObtenerClon("FISICA_Anaya");
            fisB.Grupo = "B"; fisB.Salon = "A-210";
            fisB.Docente = "Bermudez";
            ((ExamenFisica)fisB).Unidad = 3;
            examenes.Add(fisB);

            var pooA = registro.ObtenerClon("POO_Guerrero");
            pooA.Grupo = "A"; pooA.Salon = "C-202";
            examenes.Add(pooA);

            // Grupo B — Ing. García López
            var pooB = registro.ObtenerClon("POO_Guerrero");
            pooB.Grupo = "B"; pooB.Salon = "C-203";
            examenes.Add(pooB);

            var ingA = registro.ObtenerClon("INGLES");
            ingA.Grupo = "A"; ingA.Salon = "D-102";
            examenes.Add(ingA);

            var ingB = registro.ObtenerClon("INGLES");
            ingB.Grupo = "B"; ingB.Salon = "D-104";
            ingB.Docente = "Lic. Juárez Morales";
            examenes.Add(ingB);

            var redA = registro.ObtenerClon("REDES_Palma");
            redA.Grupo = "A"; redA.Salon = "B-201";
            examenes.Add(redA);

            var redB = registro.ObtenerClon("REDES_Palma");
            redB.Grupo = "B"; redB.Salon = "B-203";
            examenes.Add(redB);

            var iaA = registro.ObtenerClon("IA_Alanis");
            iaA.Grupo = "A"; iaA.Salon = "C-305";
            examenes.Add(iaA);

            var iaN = registro.ObtenerClon("IA_Alanis");
            iaN.Grupo = "N1"; iaN.Salon = "C-306";
            examenes.Add(iaN);

            var iaB = registro.ObtenerClon("IA_Alanis");
            iaB.Grupo = "B"; iaB.Salon = "C-301";
            iaB.Docente = "Dr. Luna Estrada";
            examenes.Add(iaB);
